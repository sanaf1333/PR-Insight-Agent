...
function limitBullets(section, maxBullets) {
    const lines = section
        .split("\n")
...
function isMetaRiskBullet(line) {
    return /\b(prompt|wording|word budget|bullet limit|output|summary length|docsync\.ts|riskanalysis\.ts|prsummary\.ts|build log|changelog|documentation updates?)\b/i.test(line);
}
function isDocumentationSuggestionBullet(line) {
    return /\bREADME\.md\b|\.md\b/i.test(line);
}
export function limitDiff(diffText, config) {
...
export function buildSummaryAppend(existingBody, summary) {
    const cleanedBody = existingBody.trim();
    const summarySection = buildSummarySection(summary);
    const existingSummaryPattern = new RegExp(`${escapeRegex(SUMMARY_START_MARKER)}[\\s\\S]*?${escapeRegex(SUMMARY_END_MARKER)}`, "m");
...
export function buildRiskComment(riskAnalysis, note) {
    return [
        RISK_COMMENT_MARKER,
...
export function buildDocSyncComment(docSync, note) {
    return [
...
export function normalizeRiskAnalysis(text) {
    const cleaned = normalizeWhitespace(stripLeadingHeading(text));
...
export function normalizeDocSync(text) {
    const cleaned = normalizeWhitespace(stripLeadingHeading(text));
    if (/no documentation changes suggested|docs? (appear|is) aligned|documentation (appears|is) aligned/i.test(cleaned)) {
